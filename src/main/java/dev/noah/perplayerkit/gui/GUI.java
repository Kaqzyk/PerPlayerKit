...
public class GUI {
    ...
    public void OpenMainMenu(Player p) {
        ...
        for (i = 9; i < 18; ++i) {
            if (KitManager.get().getItemStackArrayById(p.getUniqueId().toString() + (i - 8)) != null) {
                menu.getSlot(i).setItem(ItemUtil.createItem(Material.LIME_SHULKER_BOX, 1, "<dark_aqua><b>Kit " + (i - 8) + "</b></dark_aqua>", "<gray>● Left click to load kit</gray>", "<gray>● Right click to edit kit</gray>"));
            } else {
                menu.getSlot(i).setItem(ItemUtil.createItem(Material.RED_SHULKER_BOX, 1, "<dark_aqua><b>Kit " + (i - 8) + "</b></dark_aqua>", "<gray>● Left click to load kit</gray>", "<gray>● Right click to edit kit</gray>"));
            }
        ...
        for (i = 18; i < 27; ++i) {
            ...
            menu.getSlot(i).setItem(ItemUtil.createItem(Material.ENDER_CHEST, 1, "<dark_aqua><b>Enderchest " + (i - 17) + "</b></dark_aqua>", "<gray>● Click to create</gray>"));
            this.addEditEC(menu.getSlot(i), i - 17);
        }
        menu.getSlot(28).setItem(ItemUtil.createItem(Material.NETHER_STAR, 1, "<green><b>KIT ROOM</b></green>"));
        menu.getSlot(29).setItem(ItemUtil.createItem(Material.BOOKSHELF, 1, "<yellow><b>PREMADE KITS</b></yellow>"));
        menu.getSlot(30).setItem(ItemUtil.createItem(Material.OAK_SIGN, 1, "<green><b>INFO</b></green>", "<gray>● Click a kit slot to load your kit</gray>", "<gray>● Right click or click the book to edit</gray>", "<gray>● Share kits with /sharekit <slot></gray>"));
        menu.getSlot(32).setItem(ItemUtil.createItem(Material.REDSTONE_BLOCK, 1, "<red><b>CLEAR INVENTORY</b></red>", "<gray>● Shift click</gray>"));
        menu.getSlot(33).setItem(ItemUtil.createItem(Material.COMPASS, 1, "<green><b>SHARE KITS</b></green>", "<gray>● /sharekit <slot></gray>"));
        menu.getSlot(34).setItem(ItemUtil.createItem(Material.EXPERIENCE_BOTTLE, 1, "<green><b>REPAIR ITEMS</b></green>"));
        this.addRepairButton(menu.getSlot(34));
        this.addKitRoom(menu.getSlot(28));
        this.addPublicKitMenu(menu.getSlot(29));
        this.addClearButton(menu.getSlot(32));
        menu.setCursorDropHandler(Menu.ALLOW_CURSOR_DROPPING);
        menu.open(p);
    }
    ...
    public void OpenKitRoom(Player p, int page) {
        ...
        if (!p.hasPermission("perplayerkit.editkitroom")) {
            ...
            this.addKitRoomSaveButton(menu.getSlot(53), page);
        }
        ...
        for (i = 1; i < 6; ++i) {
            menu.getSlot(46 + i).setItem(ItemUtil.addHideFlags(ItemUtil.createItem(Material.valueOf(this.plugin.getConfig().getString("kitroom.items." + i + ".material")), "<reset>" + this.plugin.getConfig().getString("kitroom.items." + i + ".name"))));
        }
    ...
    public Menu ViewPublicKitMenu(Player p, String id) {
        ...
        ChestMenu menu = ChestMenu.builder(6).title(StyleManager.get().getPrimaryColor() + "Viewing Public Kit: " + id).redraw(true).build();
        for (i = 0; i < 54; ++i) {
            menu.getSlot(i).setItem(ItemUtil.createGlassPane());
        }
        for (i = 9; i < 36; ++i) {
    ...
    public void OpenPublicKitMenu(Player player) {
        ...
        for (int i2 = 0; i2 < publicKitList.size(); ++i2) {
            if (KitManager.get().hasPublicKit(publicKitList.get(i2).id)) {
                if (player.hasPermission("perplayerkit.admin")) {
                    menu.getSlot(i2 + 18).setItem(ItemUtil.createItem(publicKitList.get(i2).icon, 1, ChatColor.RESET + publicKitList.get(i2).name, "<gray>● [ADMIN] Shift click to edit</gray>"));
                } else {
                    menu.getSlot(i2 + 18).setItem(ItemUtil.createItem(publicKitList.get(i2).icon, 1, ChatColor.RESET + publicKitList.get(i2).name));
                }
                this.addPublicKitButton(menu.getSlot(i2 + 18), publicKitList.get(i2).id);
            } else if (player.hasPermission("perplayerkit.admin")) {
                menu.getSlot(i2 + 18).setItem(ItemUtil.createItem(publicKitList.get(i2).icon, 1, ChatColor.RESET + publicKitList.get(i2).name + " <red><b>[UNASSIGNED]</b></red>", "<gray>● Admins have not yet setup this kit yet</gray>", "<gray>● [ADMIN] Shift click to edit</gray>"));
            } else {
                menu.getSlot(i2 + 18).setItem(ItemUtil.createItem(publicKitList.get(i2).icon, 1, ChatColor.RESET + publicKitList.get(i2).name + " <red><b>[UNASSIGNED]</b></red>", "<gray>● Admins have not yet setup this kit yet</gray>"));
            }
            if (!player.hasPermission("perplayerkit.admin")) continue;
            this.addAdminPublicKitButton(menu.getSlot(i2 + 18), publicKitList.get(i2).id);
        }
        this.addMainButton(menu.getSlot(53));
        menu.getSlot(53).setItem(ItemUtil.createItem(Material.OAK_DOOR, 1, "<red><b>BACK</b></red>"));
        menu.open(player);
    ...
    public Menu createMainMenu(Player p) {
        return ChestMenu.builder(5).title(StyleManager.get().getPrimaryColor() + p.getName() + "'s Kits").build();
    }
